...
:heavy_exclamation_mark: This project builds a read-only MCP server. For full read, write, update, delete, and action capabilities and a simplified setup, check out our free [CData MCP Server for Azure Data Lake Storage (beta)](https://www.cdata.com/download/download.aspx?sku=HDZM-V&type=beta). 
## Purpose
We created this read-only MCP Server to allow LLMs (like Claude Desktop) to query live data Azure Data Lake Storage supported by the [CData JDBC Driver for Azure Data Lake Storage](https://www.cdata.com/drivers/azuredatalake/jdbc).
...
2. Download and install the CData JDBC Driver for {source}: [https://www.cdata.com/drivers/azuredatalake/download/jdbc](https://www.cdata.com/drivers/azuredatalake/download/jdbc)
3. License the CData JDBC Driver:
    ...
    * Run the command `java -jar cdata.jdbc.adls.jar --license`
    * Enter your name, email, and "TRIAL" (or your license key).
4. Configure your connection to the data source (Salesforce as an example):
    * Run the command `java -jar cdata.jdbc.adls.jar` to open the Connection String utility.
...
5. Create a `.prp` file for your JDBC connection (e.g. `azure-data-lake-storage.prp`) using the following properties and format:
